...
public class RubberDuckyIsopodEntity extends PathfinderMob implements IAnimatable {
    ...
    public ItemStack getPickedResult(HitResult target) {
        return new ItemStack(KAItems.RUBBER_DUCKY_ISOPOD_SPAWN_EGG.get());
    }
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        ...
        if (item == KAItems.BARK.get()) {
            if (random.nextFloat() > 0.9F) {
                ItemEntity molt = EntityType.ITEM.create(world);
                molt.setItem(new ItemStack(KAItems.RUBBER_DUCKY_ISOPOD_MOLT.get()));
            ...
            world.addParticle(new ItemParticleOption(ParticleTypes.ITEM,  new ItemStack(KAItems.BARK.get())), d0, d1, d2, vector3d.x, vector3d.y + 0.05D, vector3d.z);
