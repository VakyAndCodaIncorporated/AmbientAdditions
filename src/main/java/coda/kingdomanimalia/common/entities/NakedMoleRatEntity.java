...
public class NakedMoleRatEntity extends Animal implements IAnimatable {
    ...
    public ItemStack getPickedResult(HitResult target) {
        return new ItemStack(KAItems.NAKED_MOLE_RAT_SPAWN_EGG.get());
    }
    ...
    public NakedMoleRatEntity getBreedOffspring(ServerLevel world, AgeableMob animal) {
        return KAEntities.NAKED_MOLE_RAT.get().create(world);
    }
    ...
    protected SoundEvent getAmbientSound() {
        return KASounds.RAT_AMBIENT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource p_184601_1_) {
        return KASounds.RAT_HURT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return KASounds.RAT_DEATH.get();
    }
}
