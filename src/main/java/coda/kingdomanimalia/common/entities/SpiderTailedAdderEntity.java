package coda.kingdomanimalia.common.entities;

import coda.kingdomanimalia.common.registry.KAEntities;
import coda.kingdomanimalia.common.registry.KAItems;
import coda.kingdomanimalia.common.registry.KASounds;
...
public class SpiderTailedAdderEntity extends Animal implements IAnimatable {
    private <E extends IAnimatable> PlayState predicate(AnimationEvent<E> event) {
        boolean walking = !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F);
        if (walking){
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.spider_tailed_adder.slither", true));
        } else {
            event.getController().setAnimation(new AnimationBuilder().addAnimation("animation.spider_tailed_adder.idle", true));
        }
    ...
    public ItemStack getPickedResult(HitResult target) {
        return new ItemStack(KAItems.SPIDER_TAILED_ADDER_SPAWN_EGG.get());
    }
    ...
    public AgeableMob getBreedOffspring(ServerLevel world, AgeableMob animal) {
        return KAEntities.SPIDER_TAILED_ADDER.get().create(world);
    }
    ...
    protected SoundEvent getAmbientSound() {
        return KASounds.SNAKE_AMBIENT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getHurtSound(DamageSource p_184601_1_) {
        return KASounds.SNAKE_HURT.get();
    }

    @Nullable
    @Override
    protected SoundEvent getDeathSound() {
        return KASounds.SNAKE_DEATH.get();
    }
