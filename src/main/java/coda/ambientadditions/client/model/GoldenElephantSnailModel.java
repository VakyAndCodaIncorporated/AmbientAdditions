...
public class GoldenElephantSnailModel<T extends Entity> extends EntityModel<T> {
    ...
    public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        float speed = 1.0f;
        float degree = 6.0f;
        this.shell1.zRot = MathHelper.cos(limbSwing * 2.0f * 0.4F) * 2.0f * 0.2F * limbSwingAmount;
        this.head.yRot = MathHelper.cos(limbSwing * speed * 0.4F) * degree * 0.2F * limbSwingAmount;
        this.antenna.xRot = MathHelper.cos(limbSwing * speed * 0.4F) * degree * 0.2F * limbSwingAmount + 0.5F;
    }
